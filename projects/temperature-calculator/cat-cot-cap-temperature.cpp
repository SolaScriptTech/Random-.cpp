...
time_t t = time(0);
//This gets the current time using "time(0)" and stores it in variable "t"
tm* now = localtime(&t); 
cout << (now->tm_mon + 1) << "/" << now->tm_mday << "/" << (now->tm_year + 1900) << endl; 
/*Line 20 converts that time stored in variable "t" to a local tm structure (tm* now = localtime(&t);), 
and the next line prints out the current date in MM/DD/YYYY format.*/
}
//Spec A2 - This is where the word that was input by the user gets validated and if it's correct it returns true.
bool ValWord(string inputWord)  
/*
This next function takes a string inputWord, which is the word the user entered, and converts it to lowercase.
Then it checks if the inputWord matches any of the acceptable objects: "cat", "cot", "cap", or "dog".
Otherwise it returns false and the user gets sent back to the drawing board.
*/
{
...
/*
Spec B2 - This is where temps that are within the proper range get validated. If the object is a cat, cot, cap, or dog, 
it checks whether the temperature falls within its safe range. If it does, it returns true; otherwise, false.
*/
